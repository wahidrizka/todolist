...
class TodoControllerTest {
  ...
  void create_blankTitle_400() throws Exception {
    ...
    mockMvc
        .perform(post("/api/todos").contentType(MediaType.APPLICATION_JSON).content(badJson))
  ...
  @Test
  @DisplayName("GET /api/todos/{id} -> 200 OK")
  void getById_ok() throws Exception {
    TodoResponse response =
        new TodoResponse(
            ...
            Instant.parse("2025-01-01T00:00:00Z"),
            Instant.parse("2025-01-01T00:00:00Z"));
    when(todoService.findById(42L)).thenReturn(java.util.Optional.of(response));
    ...
    verify(todoService).findById(42L);
  }

  @Test
  @DisplayName("GET /api/todos/{id} -> 404 Not Found ketika tidak ada")
  void getById_notFound() throws Exception {
    when(todoService.findById(999L)).thenReturn(java.util.Optional.empty());

    mockMvc.perform(get("/api/todos/{id}", 999)).andExpect(status().isNotFound());

    verify(todoService).findById(999L);
  }
}
